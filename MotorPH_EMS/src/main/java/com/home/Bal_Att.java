    ...
    public String calculateOverallUndertime(List<Bean_Att> list) {
        ...
        for (Bean_Att bean : list) {
            if ("Logged in".equals(bean.getStatus())) {
                // Assuming the standard start time is 08:00 AM
                LocalTime logTime = bean.getLogtime().toLocalTime();
                LocalTime startTime = LocalTime.of(8, 0); // Standard start time is 8:00 AM

                // Check if logged in time is earlier than standard start time
                if (logTime.isBefore(startTime)) {
                    totalUndertimeMinutes += calculateMinutesDifference(logTime, startTime);
                }
            }
        }
